...
BackchannelStabilizedVehicle::BackchannelStabilizedVehicle(
    ...
    BackchannelStabilizedVehicle(backchannel_transceiver, backchannel_mac_address, my_mac_address, vehicle_controller, ahrs, receiver, nullptr)
{
}
...
BackchannelStabilizedVehicle::BackchannelStabilizedVehicle(
    ...
    _backchannel_id(idFrom_mac_address(backchannel_mac_address)),
    _telemetry_id(idFrom_mac_address(my_mac_address))
{
...
uint32_t BackchannelStabilizedVehicle::idFrom_mac_address(const uint8_t* mac_address)
{
...
bool BackchannelStabilizedVehicle::packet_set_offset(const CommandPacketSetOffset& packet)
{
...
bool BackchannelStabilizedVehicle::packet_control(const CommandPacketControl& packet)
{
    (void)packet;
    return false;
}

bool BackchannelStabilizedVehicle::packet_set_pid(const CommandPacketSetPID& packet)
{
    (void)packet;
    return false;
}

bool BackchannelStabilizedVehicle::packet_request_data(const CommandPacketRequestData& packet)
{
#if defined(USE_DEBUG_PRINTF_BACKCHANNEL)
...
*/
bool BackchannelStabilizedVehicle::send_packet(uint8_t sub_command)
{
#if defined(USE_DEBUG_PRINTF_BACKCHANNEL)
...
*/
bool BackchannelStabilizedVehicle::processed_received_packet()
{
    ...
    if (received_data_length > 0) {
        ...
        if (controlPacket->id == _backchannel_id) {
            ...
            case CommandPacketControl::TYPE:
                packet_control(*reinterpret_cast<const CommandPacketControl*>(_received_data_buffer_ptr));
                return true;
            case CommandPacketRequestData::TYPE:
                packet_request_data(*reinterpret_cast<const CommandPacketRequestData*>(_received_data_buffer_ptr));
                return true;
            case CommandPacketSetPID::TYPE:
                packet_set_pid(*reinterpret_cast<const CommandPacketSetPID*>(_received_data_buffer_ptr));
                return true;
            case CommandPacketSetOffset::TYPE:
                packet_set_offset(*reinterpret_cast<const CommandPacketSetOffset*>(_received_data_buffer_ptr));
                return true;
            default:
                // do nothing
                break;
